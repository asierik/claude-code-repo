...
export const planRepository = {
  ...
  setSlot(spaceId, date, slot, dishId) {
    db.prepare(
    ...
    ).run(spaceId, date, slot, dishId);
  },

  clearSlot(spaceId, date, slot) {
    db.prepare('DELETE FROM plan_entries WHERE space_id = ? AND date = ? AND slot = ?').run(
      spaceId,
      date,
      slot
    );
